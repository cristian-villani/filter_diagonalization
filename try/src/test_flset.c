...
void diag_tridiag(int, double *, double *, double *);
void lanczos(int, int, double **, double *, double *);
...
int main(void)
{
    ...
    printf("Enter matrix dimension (35 at least): ");
    if (scanf("%d%*c", &ndim) != 1 || ndim <= 34) {
    // %*c consumes the leftover newline
    ...
    /* -------------------------------
       ...
       ------------------------------- */
    // Hier is a list of alternative methods to generate the matrix:

    // generate_random_symmetric_matrix(ndim, 1.0);
    // create_matrix_with_isolated_eigenvalues(ndim, -5.0, 5.0);
    // generate_isolated_matrix(ndim, -5.0, 5.0);

    double* eigenvalues = (double*)malloc(ndim * sizeof(double));
    if (!eigenvalues) {
        fprintf(stderr, "Memory allocation failed\n");
        return 1;
    }
    // Create the spectrum with high and low density regions
    create_high_low_density_spectrum(ndim, eigenvalues);

    // Generate the transformed matrix
    generate_diagonal_transformed_matrix(ndim, eigenvalues);

    /* -----------------------------------------------------------
       Running Lanczos in order to get the approximate eigenvalues
       ---------------------------------------------------------*/
